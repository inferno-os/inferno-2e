...
xscan(Memimage *dst, Seg **seg, Seg *segtab, int nseg, int wind, Memimage *src, Point sp, int detail, int fixshift, int clipped)
{
	...
	fill = fillline;
	if(clipped && !isX && src->repl && src->ldepth == 3 && Dx(src->r)==1 && Dy(src->r)==1 && dst->ldepth == 3) {
		fill = fillcolor;
		sp.x = membyteval(src);
	}
...
static int
ycompare(const void *s0, const void *s1)
{
	long y0, y1;
...
static int
xcompare(const void *s0, const void *s1)
{
	long x0, x1;
...
static int
zcompare(const void *s0, const void *s1)
{
	long z0, z1;
