...
memarc(Memimage *dst, Point c, int a, int b, int t, Memimage *src, Point sp, int alpha, int phi)
{
	...
	figure = nil;
	mask = nil;
	wedge = allocmemimage(rect, 0);
	if(wedge == nil)
		goto Return;
	memfillcolor(wedge, 0);
	memfillpoly(wedge, bnd, i, ~0, memones, p00);
	figure = allocmemimage(rect, 0);
	if(figure == nil)
		goto Return;
	memfillcolor(figure, 0);
	memellipse(figure, p00, a, b, t, memones, p00);
	mask = allocmemimage(rect, 0);
	if(mask == nil)
		goto Return;
	memfillcolor(mask, 0);
