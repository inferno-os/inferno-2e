...
JScript: module
{
	...
	call:		fn(ex: ref Exec, func, this: ref Obj, args: array of ref Val): ref Ref;
...
init(cu: CharonUtils)
{
	...
	if(ES == nil)
		cu->raise(sys->sprint("EXInternal: couldn't load Ecmascript: %r"));
	exc := ref sys->Exception;
	if(sys->rescue("ecmascript*", exc) == sys->EXCEPTION) {
		if(dbg)
			sys->print("Unexpected error initializing ecmascript:  %s", exc.name);
		return;
	}
	ES->init();
	...
	if(me == nil)
		cu->raise(sys->sprint("EXInternal: can't load self as a ESHostobj: %r"));
	if(dbg > 1) {
...
doneevent := ScriptEvent(-1,0,0,0,0,0,0,0,0,nil);
...
makeframeex(f : ref Layout->Frame) : (ref Exec, ref Obj)
{
	...
	navobj := mknavobj(ex);
	ES->put(ex, winobj, "navigator", ES->objval(navobj));
	...
	if (f.doc != nil && f.doc.src != nil) {
		...
		reinitprop(locobj, "protocol", ES->strval(U->schemes[src.scheme] + ":"));
		reinitprop(locobj, "search", ES->strval("?" + src.query));
	}
	ES->put(ex, winobj, "location", ES->objval(locobj));

	# make an Image constructor object
...
# make a navigator object
# currently, we've decided to emulate Netscape Version 3
...
# First string in answer is error (if non-nil), second is possible
# output of document.write's.
# Currently, assume this is only called during page build.
# When page build is done, this should be called a final
# time with donebuild=1.
evalscript(f: ref Layout->Frame, s: string, donebuild: int) : (string, string)
{
	sw := top.findbyframe(f);
	if(sw == nil || sw.ex == nil)
		return ("", "");
	...
	if(sys->rescue("ecmascript*", exc) == sys->EXCEPTION) {
		if(dbg) {
			sys->print("error executing evalscript: %s\nscript=", ex.error);
			sa := array of byte s;
			sys->write(sys->fildes(1), sa, len sa);
			sys->print("\n");
		}
		return (ex.error, docwriteout);
	}
...
xfertoscriptobjs(f: ref Layout->Frame, inbuild: int)
{
	...
	for(el := f.doc.events; el != nil; el = tl el) {
		e := hd el;
		hname := "";
		case e.attid {
		Lex->Aonblur =>
	...
	if(od == nil) {
		...
		docfill(ex, od, f);
	for(frml := f.doc.forms; frml != nil; frml = tl frml) {
		frm := hd frml;
		for(fieldl := frm.fields; fieldl != nil; fieldl = tl fieldl) {
			fld := hd fieldl;
			# change value in control, if it is there yet
			if(fld.ctlid >= 0 && fld.ctlid < len f.controls) {
				ofield := getformfieldobj(ex, f.id, frm.formid, fld.fieldid);
				if(ofield == nil)
					continue;
		...
		if(inbuild) {
			...
			if(createdimages != nil) {
				for(oil := createdimages; oil != nil; oil = tl oil) {
					...
					if(ES->isstr(vsrc)) {
						u := U->makeurl(vsrc.str);
						if(u.scheme != U->UNKNOWN) {
							u.makeabsolute(di.base);
			...
			(oimages, n) := getarraywithlen(ex, od, "images");
			for(i := 0; i < n; i++) {
				oi := getobj(ex, oimages, string i);
				voldsrc := ES->get(ex, oi, "@PRIVsrc");
				vnewsrc := ES->get(ex, oi, "src");
				if(ES->isstr(voldsrc) && ES->isstr(vnewsrc)) {
					if(voldsrc.str != vnewsrc.str) {
						for(sil := sw.imgs; sil != nil; sil = tl sil) {
							si := hd sil;
							if(si.obj == oi) {
								ES->put(ex, oi, "@PRIVsrc", ES->strval(vnewsrc.str));
...
{
	if(dbg)
		sys->print("do_on %d, frameid=%d, formid=%d, fieldid=%d, anchorid=%d, x=%d, y=%d, which=%d\n",
	...
	# event target types
	TAnchor, TForm, TFormField, TImage, TWindow, Tnone: con iota;
	ttype := Tnone;
	...
	if(e.anchorid >= 0) {
		...
		if(e.fieldid > 0) {
			ttype = TFormField;
			target = getformfieldobj(ex, e.frameid, e.formid, e.fieldid);
		} else {
			ttype = TForm;
			target = oform;
		}
	...
	if(target != nil) {
		...
		E->SEonunload =>
			scrname = "onunload";
		E->SEtimeout =>
			oscript = dotimeout(ex, target, e.which);
		}
		if(scrname != "")
		...
		if(oscript != nil) {
			...
			ex.scopechain = sc;
			v := ES->call(ex, oscript, target, nil).val;
			v = ES->toBoolean(ex, v);
			ex.scopechain = oldsc;
...
get(ex: ref Exec, o: ref Obj, property: string): ref Val
{
	if(o.class == "document" && property == "cookie") {
		...
		if(target != nil) {
			...
			l := CU->getcookies(url.host, "/" + url.path);
			while(l != nil) {
				if(ans != nil)
					ans += ";";
				ans += hd l;
...
put(ex: ref Exec, o: ref Obj, property: string, val: ref Val)
{
	...
	"Window" =>
		...
		"location" =>
			...
			url.makeabsolute(target.frame.doc.base);
		}
	"Location" =>
		target = top.findbyobj(o);
		if (target == nil)
	...
	"Image" =>
		...
		if(property == "src" || property == "lowsrc") {
			target = top.findbyobj(o);
			if(target == nil)
				break;
			url = U->makeurl(str);
	...
	"document" =>
		if(property == "cookie") {
			...
			if(target != nil && (CU->config).docookies > 0) {
				url = target.frame.doc.src;
				CU->setcookie(str, url.host, "/" + url.path);
			}
			return;
		}
	}
...
call(ex: ref Exec, func, this: ref Obj, args: array of ref Val): ref Ref
{
if(dbg) { sys->print("call %x (class %s), val %s\n", func, func.class, func.val.str); }
	ans := ES->valref(ES->true);
	case func.val.str{
	"document.prototype.write" =>
		docwriteout += ES->toString(ex, ES->biarg(args, 0));
	...
	"Location.prototype.replace" =>
		v := ES->biarg(args, 0);
		if(ES->isstr(v)) {
			target := top.findbyobj(this);
			if(target == nil)
				fname := "_top";
			else
				fname = target.frame.name;
			E->evchan <-= ref Event.Ego(v.str, fname, 0, E->EGreplace);
		}
	"Window.prototype.alert" =>
		sync := chan of int;
		E->evchan <-= ref Event.Ealert(ES->toString(ex, ES->biarg(args, 0)), sync);
		<- sync;
	"Window.prototype.confirm" =>
		sync := chan of int;
		E->evchan <-= ref Event.Econfirm(ES->toString(ex, ES->biarg(args, 0)), sync);
		code := <- sync;
		if(code == 0)
			ans = ES->valref(ES->false);
	"Window.prototype.open" =>
		...
		if(u != "") {
			# Want to replace top window by navigation to u
			top.newloc = u;
			top.newloctarg = "_top";
			url := U->makeurl(u);
			# TODO: be more defensive
			# Perhaps make absolute relative to current frame
			url.makeabsolute(top.frame.doc.base);
			top.newloc = url.tostring();
		}
	"Window.prototype.prompt" =>
		sync := chan of (int, string);
		E->evchan <-= ref Event.Eprompt(ES->toString(ex, ES->biarg(args, 0)), ES->toString(ex, ES->biarg(args, 1)), sync);
		(code, input) := <- sync;
		if(code == 0)
			v := ES->null;
		else
	...
	* =>
		ES->runtime(ex, "unknown or unimplemented func "+func.val.str+" in host call");
		return nil;
	}
	return ans;
}
...
#
addtimeout(ex : ref Exec, win : ref Obj, cmd : string, ms : int) : int
{
	...
	ev := ref ScriptEvent(E->SEtimeout, sw.frame.id, -1, -1, -1, -1, 0, 0, ix, nil);
	spawn timeout(ev, ms);
	return ix;
}

dotimeout(ex : ref Exec, win : ref Obj, id : int) : ref Ecmascript->Obj
{
	if (id < 0)
		return nil;
...
{
if(dbg) { sys->print("construct %x (class %s), val %s\n", func, func.class, func.val.str); }
	case func.val.str {
	"Image" =>
		o := mkhostobj(ex, "Image");
		if(len args > 0) {
			reinitprop(o, "width", args[0]);
			if(len args > 1)
				reinitprop(o, "height", args[1]);
		}
...
getformfieldobj(ex : ref Exec, frameid, formid, fieldid: int) : ref Obj
{
	# fieldids are 1-origin, elements are 0-origin
	return getarrayelem(ex, getformobj(ex, frameid, formid), "elements", fieldid-1);
}
...
reinitprop(o: ref Obj, prop: string, v: ref Val)
{
	...
	if(i < 0) {
		...
		ex := ref Exec;
		ES->runtime(ex, "missing property " + prop); # shouldn't happen
	}
	CU->assert(i >= 0);
	o.props[i].val.val = v;
}
...
arrayput(ex : ref Exec, oarray: ref Obj, index: int, name: string, v: ref Val)
{
	...
	if(name != "" && name != "length") {
		for(i := 0; i < len name; i++) {
			c := name[i];
			if(c < '0' || c > '9') {
				# name is not all decimal digits, so OK
...
wininstant(sw: ref ScriptWin)
{
	...
	# document to be init'd by xfertoscriptobjs

	# frames[ ]
	# could delete old frames from frame array by setting "length" prop to 0
	# Just make a new one, as nothing else should have a ref to it at this point
	sw.kids = nil;
	ao := ES->mkobj(ex.arrayproto, "Array");
	ES->varinstant(ao, ES->DontEnum|ES->DontDelete, "length", ref RefVal(ES->numval(0.)));
	...
	for (swkl := sw.kids; swkl != nil; swkl = tl swkl) {
		k := hd swkl;
		# not sure if frame name should be defined as property of parent
...
docfill(ex: ref Exec, od: ref Obj, f: ref Layout->Frame)
{
	...
	if(di.forms != nil) {
		...
		(oforms, oldformslen) := getarraywithlen(ex, od, "forms");
		...
		if(oforms != nil) {
			...
			for(i := newformslen-1; i >= oldformslen; i--) {
				form := hd fl;
				fl = tl fl;
				vfrm := forminstant(ex, form, di.frameid);
				arrayput(ex, oforms, i, form.name, vfrm);
				if(form.name != "")
					ES->put(ex, od, form.name, vfrm);
			}
		}
	}
	...
	if(di.dests != nil) {
		...
		if(oanchors != nil) {
			...
			for(i := newdestslen-1; i >= oldanchorslen; i--) {
				dest := hd dl;
				dl = tl dl;
				arrayput(ex, oanchors, i, dest.name, anchorinstant(ex));
			}
		}
	}
...
forminstant(ex : ref Exec, form: ref Build->Form, frameid: int) : ref Val
{
...
anchorinstant(ex : ref Exec) : ref Val
{
...
linkinstant(ex: ref Exec, anchor: ref Build->Anchor) : ref Val
{
	...
	if(u != nil) {
		...
		pathname := u.pstart + u.path;
		reinitprop(olink, "pathname", ES->strval(pathname));
		if(u.port != "")
			reinitprop(olink, "port", ES->strval(u.port));
		reinitprop(olink, "protocol", ES->strval(U->schemes[u.scheme] + ":"));
		if(u.query != "")
...
{
	return ref ScriptWin(f, ex, loc, ES->objval(ex.global), par, nil, nil, "", "", 1);
}
