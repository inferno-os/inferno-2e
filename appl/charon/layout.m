...
#			onchange:	int;		# true if want onchange event
		...
		Cprogbox =>
			state:	int;			# Pstart, etc.
			pcnt:		int;			# percent done
			bsid:		int;			# bytesource id
			src:		string;		# bytesource req.url
			err:		string;		# possible error string
		Clabel =>
			s:		string;
	}
	...
	newprogbox: fn(f: ref Frame) : ref Control;
	newlabel: fn(f: ref Frame, s: string) : ref Control;
	disable: fn(b: self ref Control);
	enable: fn(b: self ref Control);
	losefocus: fn(b: self ref Control, popupactive: int);
	gainfocus: fn(b: self ref Control, popupactive: int);
	scrollset: fn(sc: self ref Control, v1, v2, vmax, mindelta: int);
...
CAbuttonpush, CAflyover, CAreturnkey, CAtabkey, CAkeyfocus, CAselected, CAchanged: con iota;
