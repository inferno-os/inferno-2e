...
attrinfo := array[LX->Numattrs] of {
	...
	LX->Aonfocus => AGEN, LX->Aonkeypress => AGEN, LX->Aonkeyup => AGEN,
	LX->Aonload => AGEN, LX->Aonmousedown => AGEN, LX->Aonmousemove => AGEN,
	LX->Aonmouseout => AGEN, LX->Aonmouseover => AGEN,
	LX->Aonmouseup => AGEN, LX->Aonreset => AGEN, LX->Aonselect => AGEN,
...
TABSP: con 5;			# default cellspacing for tables
TABPAD: con 1;		# default cell padding for tables
...
{
	di := f.doc;
	ts := TokenSource.new(bs, di.chset, mtype);
	...
	return ref ItemSource(ts, mtype, di, f, psstk, 0, 0, 0, 0, nil, nil, nil, nil);
}
...
ItemSource.getitems(is: self ref ItemSource) : ref Item
{
	...
	for(;; toki++) {
		...
		LX->Ta =>
			...
			if(href != nil) {
				di.anchors = ref Anchor(++is.nanchors, name, href, target, evl) :: di.anchors;
			...
			if(name != nil) {
				...
				additem(ps, Item.newspacer(ISPnull), tok);
		...
		LX->Tframe =>
			...
			kd.framebd = aintval(tok, LX->Aframeborder, 1);
			kd.flags = atabval(tok, LX->Ascrolling, fscroll_tab, kd.flags);
			norsz := aflagval(tok, LX->Anoresize);
			if(norsz)
				kd.flags |= FRnoresize;
			...
			if(usemap != "") {
				...
				if(!S->prefix("#", usemap)) {
					...
					if(ps.curanchor == 0) {
						...
						di.anchors = ref Anchor(++is.nanchors, "", nil, di.target, nil) :: di.anchors;
						ps.curanchor = is.nanchors;
					}
				}
			}
		...
		LX->Toption =>
			...
			option := ref Option(aflagval(tok, LX->Aselected),
						val, "");
		...
		LX->Tscript =>
#sys->print("doscripts = %d, di.hasscripts = %d, di.scripttype = %d\n", doscripts, di.hasscripts, di.scripttype);
			if(doscripts) {
				if(!di.hasscripts) {
					if(di.scripttype == CU->TextJavascript) {
						if(CU->J != nil)
							di.hasscripts = 1;
					}
				}
			...
			if(!di.hasscripts) {
				...
				# TODO: change language attr for valid javascript versions
				# (currently we don't do JavaScript1.2, Netscape 4's version)
				script := "";
				scripttoki := toki;
				if(scriptsrc != nil) {
					# TODO: fetch the script (put in a di.needsubords list
					# and let layout get it, or add mechanism to getting
					# to allow waitreq to specify the desired src)
					if(warn)
						sys->print("warning: non-local <SCRIPT> ignored\n");
				}
				else {
					(script, toki) = getpcdata(toks, toki);
				}
				if(script != "") {
					(err, replace) := J->evalscript(is.frame, script, 0);
					if(err != "") {
						if(warn)
		...
		LX->Ttd or LX->Tth =>
			...
			c := Tablecell.new(len curtab.cells + 1, # cell id
				...
				makedimen(tok, LX->Awidth),
				aintval(tok, LX->Aheight, 0),
				Background(nil, color(aval(tok, LX->Abgcolor), tr.background.color)),
				flags);
		...
		LX->Ttextarea =>
			...
			field := Formfield.new(Ftextarea,
					++is.curform.nfields,	# fieldid
					is.curform,	# form
					aval(tok, LX->Aname),
					"",				# value
					0, 0);				# size, maxlength
			field.rows = aintval(tok, LX->Arows, 3);
			field.cols = aintval(tok, LX->Acols, 50);
			is.curform.fields = field :: is.curform.fields;
	...
	outerps.items = Item.newspacer(ISPnull);
	outerps.lastit = outerps.items;
	is.psstk = psstk;
	if(doscripts && di.hasscripts) {
		# transfer any remaining objects into script engine
		J->evalscript(is.frame, "", 1);
	}
	if(dbg) {
		if(ans == nil)
...
lexstring(s: string) : array of ref Token
{
	...
	ts := TokenSource.new(bs, CU->UTF_8, CU->TextHtml);
...
Pstate.new() : ref Pstate
{
	ps := ref Pstate (
			...
			nil);			# hangstk
	ps.items = Item.newspacer(ISPnull);
	ps.lastit = ps.items;
	ps.prelastit = nil;
	return ps;
}
...
addlinebrk(ps: ref Pstate, clr: int)
{
	...
	additem(ps, Item.newspacer(ISPvline), nil);
...
addnbsp(ps: ref Pstate)
{
	...
	if((ps.curstate&IFbrk) == 0)
		ps.curstate |=  IFnobrk;
	additem(ps, Item.newspacer(ISPhspace), nil);
...
aurlval(tok: ref Token, attid: int, dflt, base: ref ParsedUrl) : ref ParsedUrl
{
	...
	if(fnd && url != nil) {
		...
		ans = U->makeurl(url);
		if(ans.scheme == U->UNKNOWN) {
			if(warn)
				sys->print("warning: couldn't parse URL %s\n", url);
			ans = dflt;
		}
		else if(base != nil)
			ans.makeabsolute(base);
	}
	return ans;
}
...
Item.newimage(di: ref Docinfo, src: ref ParsedUrl, lowsrc: ref ParsedUrl, altrep: string,
	align: byte, width, height, hspace, vspace, border, ismap: int,
...
Item.newspacer(spkind: int) : ref Item
{
	return ref Item.Ispacer(nil, 0, 0, 0, 0, 0, nil, spkind);
}
