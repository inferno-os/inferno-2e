...
typetagfidqid(mtype: byte, tag, fid: int, qid: Sys->Qid)
{
	putb(mtype, 0);
	put2(tag, 1);
	put2(fid, 3);
...
errorR(tag: int, ename: string)
{
	e := array of byte ename;
	putb(Styx->Rerror, 0);
	put2(tag, 1);
	l := len e;
...
walkR(tag, fid: int, qid: Sys->Qid)
{
	typetagfidqid(Styx->Rwalk, tag, fid, qid);
}

openR(tag, fid: int, qid: Sys->Qid)
{
	typetagfidqid(Styx->Ropen, tag, fid, qid);
}

createR(tag, fid: int, qid: Sys->Qid)
{
...
attachR(tag, fid: int, qid: Sys->Qid)
{
