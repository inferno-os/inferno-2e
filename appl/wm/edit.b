...
#	"pack .m.filename -padx 10 -side left",
	"pack .m.linenum -padx 3 -side left",
	"menu .m.file.menu",
	...
	"text .b.t -width 67w -height 365 -yscrollcommand {.b.s set} -bg white",
	"bind .b.t <Button-2> {.m.edit.menu post %X %Y}",
	"bind .b.t <Motion> {.m.linenum configure -text [.b.t index current]}",
	"bind .b.t <Key> +{send c dirtied {%A}}",
	"bind .b.t <ButtonRelease-1> +{send c reindent}",
	"scrollbar .b.s -command {.b.t yview}",
	"bind . <Enter> {focus .b.t}",
	"bind .b.t <Enter> {focus .b.t}",
	"pack .m -fill x",
...
init(ctxt: ref Draw->Context, argv: list of string)
{
	...
	screen = ctxt.screen;
	(ed, wmctl) = wmlib->titlebar(screen, tkargs, "Notepad", Wmlib->Appl);
	...
	tk->cmd(ed, ".Wm_t.title configure -text 'Notepad - " + curfile);
	tk->cmd(ed, "update");
	...
	cmdloop: for(;;) {
		...
		s := <-c =>
			...
			case s {
			"exit" =>	if ( check_dirty() ) break cmdloop;
			"dirtied" =>	set_dirty(); do_limbo_check(s);
			"new" =>	if ( check_dirty() ) do_new();
			"open" =>	if ( check_dirty() ) do_open();
			"save" =>	do_save(0);
			"saveas" =>	do_save(1);
		...
		s := <-wmctl =>
			...
			task_title: string;
			if (s == "task") {
				task_title = tk->cmd(ed, ".Wm_t.title cget -text");
				if (curfile == "(New)")
					tk->cmd(ed, ".Wm_t.title configure -text Notepad");
				else
					tk->cmd(ed, ".Wm_t.title configure -text 'Notepad - " + curfile);
				tk->cmd(ed, "update");
			}
			wmlib->titlectl(ed, s);
			if (s == "task") 
				tk->cmd(ed, ".Wm_t.title configure -text '" + task_title);
		s := <-drag =>
			if ( len s < 6 )
				break;
			case s[0:5] {
				...
				if(e != nil)
				    wmlib->dialog(ed, ErrIco, "Open file", e, 0, "Ok"::nil);
			    "font=" =>
				tk->cmd(ed, "raise .; .b.t configure -font " +
									s[5:]);
				tk->cmd(ed, ".b.t configure -width 80w -height 24h");
			}
		}
		tk->cmd(ed, "update");
		e = tk->cmd(ed, "variable lasterror");
...
#	tk->cmd(ed, "bind .b.t <Key> -{send c dirtied}");
}
...
do_limbo()
{
	...
	if ( is_limbo )
		...
		tk->cmd(ed, ".b.t tag remove comment 1.0 end");
		tk->cmd(ed, ".b.t tag remove keyword 1.0 end");
	}
}
...
#
	...
	case char {
		...
		if ( in_comment ) {
			...
			tk->cmd(ed, ".b.t tag remove comment insert-1chars");
			in_comment = 0;
		}
		else
			look_keyword(this_word);
	...
	# to have one less indent.
		if ( first_char ) {
			current := int tk->cmd(ed, ".b.t index insert");
			tk->cmd(ed, ".b.t delete " +
						string current + ".0 insert");
			add_indent();
			tk->cmd(ed, ".b.t insert insert '}");
		}
#	    ';' =>
#		last_kw_is_block = 0;
...
#			# insert point.
	    '#' =>
		# if ( ! in_quote ) {
		#	tk->cmd(ed, ".b.t tag add comment insert-1chars");
			in_comment = 1;
		# }
...
look_keyword(word : string)
{
	...
	if ( is_keyword(word) ) {
		tk->cmd(ed, ".b.t tag add keyword insert-" +
...
do_open()
{
	...
	for(;;) {
		fname := wmlib->filename(screen, ed, "", pats, path);
		if(fname == "")
			break;
		tk->cmd(ed, ".b.t delete 1.0 end");
		e := loadtfile(fname);
...
do_replace(prompt : int)
{
	...
	if ( range != "" ) {		# there's something selected
		tk->cmd(ed, ".b.t mark set insert sel.first");
	}
	...
	tk->cmd(ed, ".b.t delete sel.first sel.last");
	tk->cmd(ed, ".b.t insert insert " + wmlib->tkquote(replacewith));
}

do_replaceall()
{
	cur := tk->cmd(ed, ".b.t index insert");
	if ( cur == "" || cur[0] == '!' )
		return;
	dirt := 0;
	if ( searchfor == "" )		# no search string
	...
	for ( ix := "1.0"; len ix > 0 && ix[0] != '!'; ) {
		ix = tk->cmd(ed, ".b.t search -- " + srch + " " + ix + " end");
		if ( ix == "" || len ix <= 1 || ix[0] == '!')
			break;
		tk->cmd(ed, ".b.t delete " + ix + " " + ix + "+" +
						string(len searchfor) + "c");
		if ( replacewith != "" ) {
			tk->cmd(ed, ".b.t insert " + ix + " " + repl);
			ix = tk->cmd(ed, ".b.t index " + ix + "+" +
					string(len replacewith) + "c");
		}
		dirt++;
	}
	tk->cmd(ed, ".b.t mark set insert " + cur);
	if ( dirt > 0 )
		set_dirty();
}
...
getstring(t : ref tk->Toplevel, label : string) : string
{
	retval : string;

	tk->cmd(t, "bind . <Enter> {}");
	tk->cmd(t, "bind .b.t <Enter> {}");
	retval = wmlib->getstring(t, label);
	tk->cmd(t, "bind . <Enter> {focus .b.t}");
	tk->cmd(t, "bind .b.t <Enter> {focus .b.t}");
	return retval;
}
...
mark_keyw_comm()
{
	...
	last := int tk->cmd(ed, ".b.t index end");
	for ( i := 1; i <= last; i++ ) {
		quote = 0;
		word = "";
...
ll :		for ( j := 0; j < l; j++ ) {
			...
			case c {
			    '#' =>
				tk->cmd(ed, sys->sprint(".b.t tag add comment" +
					" %d.%d %d.%d", i, j, i, l));
				break ll;
			    '\'' or '\"' =>
				if ( j != 0 && line[j-1] == '\\' )
...
add_indent()
{
	for ( i := len indent; i >= 8; i -= 8 )
		tk->cmd(ed, ".b.t insert insert '" + tabs[8]);
	tk->cmd(ed, ".b.t insert insert '" + tabs[i]);
}
#
...
re_indent()
{
	pos := tk->cmd(ed, ".b.t index insert");
