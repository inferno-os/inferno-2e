...
{
	(t, titlectl) := wmlib->titlebar(ctxt.screen, tkargs+SETFONT, Name, Wmlib->Appl);
	...
	(fname, addr) := <-plumbc =>
		...
		if(i == len files){
			if(addr != "")
				spawn brutus(ctxt, tkargs, fname+":"+addr);
			else
				spawn brutus(ctxt, tkargs, fname);
		}
	}
}
...
brutus(ctxt: ref Context, tkargs: string, filename: string)
{
	...
	if ((wh := load Tkwh Tkwh->PATH) != nil)
	  brutus_cfg = wh->tkwhset(brutus_cfg, "text .ft.t -height", tkargs);
