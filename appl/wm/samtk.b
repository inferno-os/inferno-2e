...
newflayer(tag, tp: int): ref Flayer
{
	...
	t := tk->toplevel(ctxt.ctxt.screen, "-borderwidth 1 -relief raised");
	...
	n := chanadd();
	ctxt.titlesel[n] = wmlib->titlebar(t, "SamTerm", wmlib->Appl);
	...
	if (tp) {
		lines = 8;
		tklib->tkcmds(t, tksam1);
		mkmenu2c(t);
	} else {
		lines = 20;
		tklib->tkcmds(t, tkwork1);
		mkmenu2(t);
	}
	mkmenu3(t);
	tklib->tkcmds(t, tkcmdlist);
