...
noinit: con "pslib not properly initialized";
...
init(env : ref Draw->Context,d: ref Toplevel,box: int,deb : int) : string
{
...
preamble(ioutb : ref Iobuf, bb: Rect) : string
{
...
trailer(ioutb : ref Iobuf,pages : int) : string
{
	if (!started) return noinit;
	ioutb.puts("%%Trailer\n%%Pages: "+string pages+"\n%%EOF\n");
	return "";
...
image2psfile(ioutb : ref Iobuf, im : ref Draw->Image, dpi: int) : string
{
	r := im.r;
	width := r.dx();
	height := r.dy();
