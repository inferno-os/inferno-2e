...
writeheader(fd: ref Iobuf, image: ref Image)
{
	# Header
	fd.puts("GIF89a");
	...
	fd.putb(byte ((1<<7)|(3<<4)|((1<<image.ldepth)-1)));
	fd.putb(byte 0);	# white background (doesn't matter anyway)
	fd.putb(byte 0);	# pixel aspect ratio - unused

	# Global Color Table
	getcolormap(image);
	fd.write(colormap[image.ldepth], len colormap[image.ldepth]);
}
...
getcolormap(image: ref Draw->Image)
{
	if(colormap != nil)
		return;
	colormap = array[4] of array of byte;
...
encode(fd: ref Iobuf, image: ref Image): string
{
	...
	data := array[image.r.dy()*(2+(image.r.dx()>>(3-image.ldepth)))] of byte;
