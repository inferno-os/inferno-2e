...
init(): string
{
	...
	if(daytime == nil)
		return "load DayTime module failed";
	...
	if(pkcs == nil || pkcs->init() != "")
		return "load PKCS module failed";
...
parse:
	for(;;) {
		...
		(ok, tag, constr, i, n) := der_dec1(a, 0, len a);
		if(!ok || n != len a || !constr || 
			tag.class != Universal || tag.num != SEQUENCE)
			break parse;
		s := ref Signed;
		# SIGNED sequence
		istart := i;
		(ok, tag, constr, i, n) = der_dec1(a, i, len a);
		if(!ok || n == len a)
			break parse;
		s.tobe_signed = a[istart:n];
		# AlgIdentifier
		istart = n;
		(ok, tag, constr, i, n) = der_dec1(a, n, len a);
		if(!ok || n == len a 
			|| !constr || tag.class != Universal || tag.num != SEQUENCE) {
			if(X509_DEBUG)
				log("signed: der data: " + 
				sys->sprint("ok=%d, n=%d, constr=%d, class=%d, num=%d", 
				ok, n, constr, tag.class, tag.num));
			break parse;
		}
		...
		(ok, tag, constr, i, n) = der_dec1(a, n, len a);
		if(!ok || n != len a
			|| constr || tag.class != Universal || tag.num != BIT_STRING) {
			if(X509_DEBUG)
...
# [private]
# decode ASN1 one record at a time and return (err, tag, constr, start of data, 
# end of data) for indefinite length, the end of data is same as 'n'

der_dec1(a: array of byte, i, n: int): (int, Tag, int, int, int)
{
	constr, length: int;
	tag: Tag;
	ok := 1;
	(ok, i, tag, constr) = der_dectag(a, i, n);
	if(ok) {
	...
	return (ok, tag, constr, i, i+length);
}

# [private]
# der tag decode

der_dectag(a: array of byte, i, n: int): (int, int, Tag, int)
{
	ok := 1;
...
pack:
	for(;;) {
		...
		all := ref Elem(Tag(Universal, SEQUENCE), ref Value.Seq(lseq));
		return asn1->encode(all);
	}
...
{
	# version is marked by [0]; no version is required if
	# default version (v1) is used
	ver := 0;
	if(e.tag.class == ASN1->Context && e.tag.num == 0) {
		ok := 0;
		(ok, ver) = e.is_int();
		if(!ok || ver < 0 || ver > 2)
			return (0, -1);
...
{
	return ref Elem(Tag(Universal, INTEGER), ref Value.Int(v));
}
...
{
	return ref Elem(Tag(Universal, INTEGER), ref Value.BigInt(sn.iptobytes()));
}
...
pack_alg(a: ref AlgIdentifier): ref Elem
{
	if(a.oid != nil) {
		el: list of ref Elem;
		el = ref Elem(Tag(Universal, ASN1->OBJECT_ID), ref Value.ObjId(a.oid)) :: nil;
		if(a.parameter != nil)  {
			el = ref Elem(
				Tag(Universal, BIT_STRING), 
		...
		return ref Elem(Tag(Universal, SEQUENCE), ref Value.Seq(el));
	}
	return nil;
}
...
pack_name(n: ref Name): ref Elem
{
	...
	return ref Elem(Tag(Universal, SEQUENCE), ref Value.Seq(l));
}
...
pack_rdname(r: ref RDName): ref Elem
{
	...
	return ref Elem(Tag(Universal, ASN1->SET), ref Value.Set(el));
}
...
pack_ava(a: ref AVA): ref Elem
{
	...
	el = ref Elem(Tag(Universal, ASN1->GeneralString), ref Value.String(a.value)) :: el;
	el = ref Elem(Tag(Universal, ASN1->OBJECT_ID), ref Value.ObjId(a.oid)) :: el;	
	return ref Elem(Tag(Universal, SEQUENCE), ref Value.Seq(el));
}
...
{
	return ref Elem(Tag(Universal, ASN1->BIT_STRING), ref Value.BitString(0,u));
}
...
pack_extension(e: ref Extension): ref Elem
{
	...
	if(e.oid == nil || (e.critical !=0 && e.critical != 1) || e.value == nil)
		return nil;
	# SEQUENCE order
	el = ref Elem(Tag(Universal, OCTET_STRING), ref Value.Octets(e.value)) :: el;
	el = ref Elem(Tag(Universal, BOOLEAN), ref Value.Bool(e.critical)) :: el;
	el = ref Elem(Tag(Universal, ASN1->OBJECT_ID), ref Value.ObjId(e.oid)) :: el;
	return ref Elem(Tag(Universal, SEQUENCE), ref Value.Seq(el));
}
...
parse:
	for(;;) {
		...
		if(!ok)
			break parse;
		c.user_cert = IPint.bytestoip(uc);
		el = tl el;
...
pack_revoked(r: ref RevokedCert): ref Elem
{
	...
	if(r.user_cert == nil)
		return nil;
	el = ref Elem(Tag(Universal, INTEGER), 
			ref Value.BigInt(r.user_cert.iptobytes())
		) :: el;
	return ref Elem(Tag(Universal, SEQUENCE), ref Value.Seq(el));
}
...
ExtClass.tostring(et: self ref ExtClass): string
{
	...
	AuthorityKeyIdentifier =>
		...
		s += "\n\tserial_number = " + bastr(t.serial_number.iptobytes());
	SubjectKeyIdentifier =>
...
encode_authorityKeyIdentifier(c: ref ExtClass.AuthorityKeyIdentifier): (string, array of byte)
{
	...
	if(c.serial_number != nil) {
		(ok, e) := pack_context(
				ref Elem(
					Tag(ASN1->Universal, ASN1->INTEGER),
					ref Value.BigInt(c.serial_number.iptobytes())
				),
				2
			);
		if(!ok)
	...
	if(c.id != nil) {
		(ok, e) := pack_context(
				ref Elem(
					Tag(ASN1->Universal, OCTET_STRING),
	...
	return asn1->encode(ref Elem(Tag(ASN1->Universal, ASN1->SEQUENCE), ref Value.Seq(el)));
}
...
encode_subjectKeyIdentifier(c: ref ExtClass.SubjectKeyIdentifier): (string, array of byte)
{
	if(c.id == nil)
		return ("syntax error", nil);
	e := ref Elem(Tag(ASN1->Universal, ASN1->OCTET_STRING), ref Value.Octets(c.id));
...
{
	el: list of ref Elem;
	el = ref Elem(Tag(ASN1->Universal, ASN1->INTEGER), ref Value.Int(c.depth)) :: nil;
	el = ref Elem(Tag(ASN1->Universal, ASN1->BOOLEAN), ref Value.Bool(1)) :: el;
	e := ref Elem(Tag(ASN1->Universal, ASN1->SEQUENCE), ref Value.Seq(el));
	return asn1->encode(e);
}
...
encode_privateKeyUsage(c: ref ExtClass.PrivateKeyUsage): (string, array of byte)
{
	...
	if(p.not_after > 0) {
		...
		e = ref Elem(Tag(ASN1->Universal, ASN1->GeneralizedTime), ref Value.String(t));
		(ok, e) = pack_context(e, 1);
		if(!ok)
...
encode_policyMapping(c: ref ExtClass.PolicyMapping): (string, array of byte)
{
	...
	while(pl != nil) {
		...
		if(a == nil || b == nil)
			return ("policy mapping: incomplete data", nil);
		be := ref Elem(Tag(ASN1->Universal, ASN1->OBJECT_ID), ref Value.ObjId(b));
		ae := ref Elem(Tag(ASN1->Universal, ASN1->OBJECT_ID), ref Value.ObjId(a));
		pel = ref Elem(
			Tag(ASN1->Universal, ASN1->SEQUENCE), 
	...
	while(pl != nil) {
		...
		if(p.qualifiers != nil) {
			...
			while(ql != nil) {
				...
				if(pq.value != nil) {
					pqseq = ref Elem(
							Tag(ASN1->Universal, ASN1->OCTET_STRING),
							ref Value.Octets(pq.value)
					) :: nil;
				}
				pqseq = ref Elem(
						Tag(ASN1->Universal, ASN1->OBJECT_ID),
...
pack_gsubtree(g: ref GSubtree): (int, ref Elem)
{
	...
	if(g.base == nil)
		return (0, nil);
	if(g.max != 0) {
		e = ref Elem(Tag(ASN1->Universal, ASN1->INTEGER), ref Value.Int(g.max));
		(ok, e) = pack_context(e, 1);
		if(!ok)
			return (0, nil);
		el = e :: nil;
	...
	if(g.min != 0) {
		e = ref Elem(Tag(ASN1->Universal, ASN1->INTEGER), ref Value.Int(g.min));
		(ok, e) = pack_context(e, 0);
		if(!ok)
			return (0, nil);
		el = e :: el;
...
encode_policyConstraints(c: ref ExtClass.PolicyConstraints): (string, array of byte)
{
	...
	ok := 1;
	if(c.inhibit > 0) {
		e := ref Elem(Tag(ASN1->Universal, ASN1->INTEGER), ref Value.Int(c.inhibit));
		(ok, e) = pack_context(e, 1);
		if(!ok)
	...
	if(c.require > 0) {
		e := ref Elem(Tag(ASN1->Universal, ASN1->INTEGER), ref Value.Int(c.require));
		(ok, e) = pack_context(e, 0);
		if(!ok)
			return ("policy constraints: encoding error", nil);
		el = e :: el;
...
{
	e := ref Elem(Tag(ASN1->Universal, ASN1->INTEGER), ref Value.Int(c.curr));
	return asn1->encode(e);
}
...
{
	e := ref Elem(
			Tag(ASN1->Universal, ASN1->BIT_STRING), 
...
{
	e := ref Elem(Tag(ASN1->Universal, ASN1->OBJECT_ID), ref Value.ObjId(c.oid));
	return asn1->encode(e);
}
...
{
	e := ref Elem(
			Tag(ASN1->Universal, ASN1->GeneralizedTime), 
	...
	: (string, array of byte)
{
	...
	if(c.indirect_crl != 0) { # no encode for DEFAULT
		e := ref Elem(
				Tag(ASN1->Universal, ASN1->BOOLEAN), 
...
parse:
	for(;;) {
		...
		if(!ok)
			break parse;
		return ("", ref ExtClass.DeltaCRLIndicator(IPint.bytestoip(b)));
	}
...
encode_deltaCRLIndicator(c: ref ExtClass.DeltaCRLIndicator): (string, array of byte)
{
	e := ref Elem(
			Tag(ASN1->Universal, ASN1->INTEGER), 
			ref Value.BigInt(c.number.iptobytes())
		);
	return asn1->encode(e);
}
...
pack_context(e: ref Elem, num: int): (int, ref Elem)
{
	(err, b) := asn1->encode(e);
	if(err == "") 
		return (1, ref Elem(Tag(ASN1->Context, num), ref Value.Octets(b)));
	return (0, nil);
}
...
pack_gname(gn: ref GeneralName): (int, ref Elem)
{
	...
	otherName => 
			e = ref Elem(
					Tag(ASN1->Universal, ASN1->GeneralString),
	...
	rfc822Name =>
			e = ref Elem(
					Tag(ASN1->Universal, ASN1->IA5String),
					ref Value.String(g.str)
				); 
			(ok, e) = pack_context(e, 1);
			if(!ok)
...
parse_attr(e: ref Elem): (int, ref Attribute)
{
	return (0, nil);
}
...
pack_attr(a: ref Attribute): (int, ref Elem)
{
	return (0, nil);
}
