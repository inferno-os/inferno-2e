	...
	r: ref Record): string
{
	...
	if(ctx.status & SSL3_RECORD) {
		h = 5;
		if(q.cipherState.block_size > 1) { # at least one byte of padding
...
set_queue(ctx: ref Context, q: ref RecordQueue, mac, key: array of byte): string
{
	...
	* =>
	...
	SSL_RC4 =>
		cs := setupRC4state(key, len key);
		q.cipherState = ref CipherState.rc4(1, cs);
	SSL_DES_CBC or
	SSL_RC2_CBC or
	SSL_IDEA_CBC or
	SSL_3DES_EDE_CBC or
...
do_server_cert(hm: ref Handshake.Certificate, ctx: ref Context)
{
	...
	RSA =>
		...
		if((key.pk.modulus.bits() > 512 && ctx.sel_ciph.is_exportable)
...
install_server_xkey(a: array of byte, keyx: ref KeyExAlg): (string, int)
{
	...
	RSA =>
		...
		if(err == "") {
			modlen := len peer_tmp.modulus.iptobytes();
...
dh_params_encode(dh: ref DHPublicKey): (array of byte, string)
{
	if(dh != nil && dh.param != nil) {
		pb := dh.param.prime.iptobytes();		
		gb := dh.param.base.iptobytes();
		yb := dh.pk.iptobytes();	
...
dh_params_decode(a: array of byte): (ref DHPublicKey, int)
{
	...
	for(;;) {
		...
		param := ref DHParams(IPint.bytestoip(p), IPint.bytestoip(g), 0);
		return (ref DHPublicKey(param, IPint.bytestoip(Ys)), i);
	}
	return (nil, i);
}
...
{
	if(rsa_params != nil) {
		mod := rsa_params.modulus.iptobytes();
		exp := rsa_params.exponent.iptobytes();
		if(mod != nil || exp != nil) {
			n := 4 + len mod + len exp;
			a := array [n] of byte;
			i := 0;
...
rsa_params_decode(a: array of byte): (ref RSAParams, int, string)
{
	...
	for(;;) {
		...
		i += n;
		m := i;
		modulus := IPint.bytestoip(mod);
		exponent := IPint.bytestoip(exp);
...
v3tov2specs(suites: array of byte): array of byte
{
	...
	for(i := 0; i < n;) {
		...
		for(j := 0; j < m; j++) {
			a = SSL3_Suites[j]; 
			if(a[0]==suites[0] && a[1]==suites[1]) 
				break;
		}
		if(j == m) {
			if(SSL_DEBUG)
				log("ssl3: convert v3 suite to v2 kind: unknown cipher");
			continue;
		}
		case j {
		RSA_EXPORT_WITH_RC4_40_MD5 => 	
