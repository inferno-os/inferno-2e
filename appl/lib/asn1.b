...
ber_decode(a: array of byte, i, n: int) : (int, int, ref Elem)
{
	...
	err, constr, length: int;
	tag : Tag;
	val : ref Value;
	elem : ref Elem = nil;
	(err, i, tag, constr) = tag_decode(a, i, n);
	if(err == OK) {
		...
		if(err == OK) {
			...
			if(tag.class == Universal)
				(err, i, val) = value_decode(a, i, n, length, tag.num, constr);
			else
...
tag_decode(a: array of byte, i, n: int) : (int, int, Tag, int)
{
	err := OK;
