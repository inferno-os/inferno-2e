...
runtime(ex: ref Exec, s: string)
{
	...
	sys->raise("ecmascript runtime error");
	exit;	# never reached
}
...
mkobj(proto: ref Obj, class: string): ref Obj
{
	if(class == nil)
		class = "Object";
	return ref Obj(nil, proto, nil, nil, nil, class, nil);
}
...
valcheck(ex: ref Exec, v: ref Val, hint: int)
{
	...
	|| hint != NoHint && v.ty != hint)
		runtime(ex, "bad value generated by host object");
}
...
esdefaultval(ex: ref Exec, o: ref Obj, ty: int, force: int): ref Val
{
	...
	if(!force && o.host != nil && o.host != me){
		...
		valcheck(ex, v, NoHint);
		if(!isprimval(v))
			runtime(ex, "host object returned an object to [[DefaultValue]]");
		return v;
	}
...
esconstruct(ex: ref Exec, func: ref Obj, args: array of ref Val): ref Obj
{
	...
	if(func.construct == nil)
		runtime(ex, "new must be applied to a constructor object");
	if(func.host != nil)
		...
		if(o == nil)
			o = ex.objproto;
		this := mkobj(o, "Object");
		o = getobj(getValue(ex, escall(ex, func, this, args)));
	...
	if(o == nil)
		runtime(ex, func.val.str+" failed to generate an object");
	return o;
}

escall(ex: ref Exec, func, this: ref Obj, args: array of ref Val): ref Ref
{
	if(func.call == nil)
	...
	r: ref Ref = nil;
	if(func.host != nil){
		r = func.host->call(ex, func, this, args);
		if(r.isref && r.name == nil)
			runtime(ex, "host call returned a bad reference");
		else if(!r.isref)
	...
	exe := ref *ex;
	exe.this = this;
	exe.scopechain = actobj :: ex.scopechain;

	(k, v) := exec(exe, func.call.code);
...
{
	return ref Val(TNum, r, nil, nil);
}

strval(s: string): ref Val
{
	return ref Val(TStr, 0., s, nil);
}

objval(o: ref Obj): ref Val
{
	return ref Val(TObj, 0., nil, o);
}
...
{
	if(!r.isref)
		runtime(ex, "not a reference: " + r.name);
	b := r.base;
	if(b == nil)
		b = ex.global;
...
toPrimitive(ex: ref Exec, v: ref Val, ty: int): ref Val
{
	...
	if(v.ty == TObj)
		runtime(ex, "toPrimitive returned an object");
	return v;
}
...
toBoolean(ex: ref Exec, v: ref Val): ref Val
{
	...
	TObj =>
		break;
	* =>
		runtime(ex, "unknown type in toBoolean");
	}
	return true;
}
...
toNumber(ex: ref Exec, v: ref Val): real
{
	...
	TNum =>
		return v.num;
	TStr =>
		(si, r) := parsenum(ex, v.str, 0, ParseReal|ParseHex);
		if(si != len v.str)
			r = Math->NaN;
		return r;
	TObj =>
	...
	* =>
		runtime(ex, "unknown type in toNumber");
		return 0.;
	}
}
...
toString(ex: ref Exec, v: ref Val): string
{
	...
	* =>
		runtime(ex, "unknown type in ToString");
		return "";
	}
}
...
toObject(ex: ref Exec, v: ref Val): ref Obj
{
	case v.ty{
	TUndef =>
		runtime(ex, "can't convert undefined to an object");
	TNull =>
		runtime(ex, "can't convert null to an object");
	TBool or
	TStr or
	TNum =>
	...
	TObj =>
		return v.obj;
	* =>
		runtime(ex, "unknown type in toObject");
		return nil;
	}
	return nil;
}
