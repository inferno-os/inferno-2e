...
mashcmd(e: ref Env, l: list of string)
{
	...
	"string" =>
		...
		e.output(wmlib->getstring(gtop, hd l));
		focus(e);
	"taskbar" =>
		if (len l != 1) {
...
{
	return wmlib->tkquote("send mash " + wmlib->tkquote(c));
}
...
{
	e.output(wmlib->filename(gctxt.screen, gtop, title, pats, dir));
}
...
dialog(e: ref Env, l: list of string)
{
	title := hd l;
	l = tl l;
	msg := hd l;
	l = tl l;
	x := wmlib->dialog(gtop, nil, title, msg, int hd l, tl l);
	e.output(string x);
	focus(e);
}
