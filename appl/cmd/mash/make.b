...
dbuf:		array of Sys->Dir;		# dir read buffer
...
loaddir(s: string)
{
	...
	if (fd == nil)
		return;
	if (dbuf == nil)
		dbuf = array[DSIZE] of Sys->Dir;
	while ((c := sys->dirread(fd, dbuf)) > 0) {
		for (i := 0; i < c; i++) {
	...
	while (l != nil) {
		...
		if (s[0] == '-') {
			...
			* =>
			...
			x := ref Sys->Exception;
			if (sys->rescue(mashlib->FAILPAT, x) == Sys->EXCEPTION) {
				makelevel--;
