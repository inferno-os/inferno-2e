...
print, fprint, open, bind, mount, dial, sleep: import sys;
...
rootfs(argv: list of string) : int
{
	...
	if(ai == nil){
		...
		if(manf!=nil){
			...
			reg := load Command "/dis/register.dis";
			if(reg!=nil){
				reg->init(nil, argv);
				ai = kr->readauthinfo("/nvfs/default");
				if(ai == nil)
				...
				sys->print("failed to load /dis/register.dis\n");
			}
		}
		else{
...
#	} else {
#		ssl := load SSL "/dis/ssl.dis";
#		newfd := ssl->push(c.dfd, "sha", secret[0:8], secret[0:8]);
#		c.dfd = newfd;
	}
...
#
#	screen = Screen.allocate(disp, menustyle.screencolor, 0);
#	screen.image.draw(screen.image.r, menustyle.screencolor, ones, (0, 0));
...
#	c.draw();
	...
	if(ok >= 0) {
		print("stat(services/namespace.init) succeeded\n");
...
#	sys->bind("#U/dis", "/dis", sys->MAFTER);
	...
	mux = load Mux "/wm/logon.dis";
	if(mux == nil) {
		print("init: load /wm/logon.dis: %r, halted ...");
		exit;
	}
