...
fncheck(Decl *d)
{
	...
	n->right = scheck(n->right, d->ty->tof);
	if(labdep != 0)
...
scheck(Node *n, Type *ret)
{
	...
	for(; n != nil; n = n->right){
		...
		case Ofor:
			 ...
			 */
			pushlabel(n);
			right->right = scheck(right->right, ret);
			right->left = scheck(right->left, ret);
			labdep--;
...
pickcheck(Node *n, Type *ret)
{
	...
	for(qs = n->right; qs != nil; qs = qs->right){
		...
		if(qt != nil)
			argty->tof = qt->ty;
		qs->left->right = scheck(qs->left->right, ret);
