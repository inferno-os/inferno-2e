...
tktxyparse(Tk* tk, char **parg, Point *p)
{
	...
	*parg = tkword(tk->env->top, *parg, buf, buf+Tkmaxitem);
	if(*buf == '\0') {
		free(buf);
		return TkOparg;
	}
...
/*	Do this in tktextevent, instead, so as not to propagate synthetic
 *	events back to the parent.	DBK */
...
tktextevent(Tk *tk, int event, void *data)
{
	...
	if(event == TkLeave && tkt->mouse != nil) {
		...
		tkt->mouse = nil;
		tkt->focus = nil;
/*		return;	*/ /*	so tksubdeliver at end is done - DBK */
	}
	else	/* DBK */
	if(event & TkEmouse) {
	...
	else	/* { move call to tksubdeliver outside of this case - DBK */
		if(event == TkFocusin) 
...
tktprintitem(TkTitem *i)
{
	...
	case TkTrune:
		...
		break;
	case TkTnewline:
		print("<nl:%lux>", i->iline);
		break;
	case TkTcontline:
		print("<cont:%lux>", i->iline);
		break;
	case TkTtab:
		print("<tab>");
		break;
...
tktprintline(TkTline *l)
{
	...
	print("line %lux: orig=(%d,%d), w=%d, h=%d, a=%d, f=%x\n\t",
...
tktprinttext(TkText *tkt)
{
	...
	for(mi = tkt->marks; mi != nil; mi=mi->next)
		print("%s{%lux} ", mi->name? mi->name : "nil", mi->cur);
	print("\n");
	print("selfirst=%lux sellast=%lux\n", tkt->selfirst, tkt->sellast);
