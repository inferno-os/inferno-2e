...
static	int	powertrace(Map*, ulong, ulong, ulong, Tracer);
...
/*
 * Don't use riscframe() to allow checking for encountering an
 * interrupt frame on the stack. I haven't figured out a machine
 * independent method of detected an interrupt frame. The only way
 * is to check for the address being the return from the call to
 * trap() in l.s.
 */
int
powertrace(Map *map, ulong pc, ulong sp, ulong link, Tracer trace)
{
	int i;
	Symbol s, f;
	ulong oldpc;
	...
	while(findsym(pc, CTEXT, &s)) {
		...
		if(s.type == 'L' || s.type == 'l' || pc <= s.value+mach->pcquant)
			...
			if (get4(map, sp, (long *) &pc) < 0)
				break;
		/* was this a interrupt */
		if (strcmp("intrvec", s.name) == 0) {
			/* Skip past interrupt frame */
			sp += 2*sizeof(ulong);
			/* get new pc from Ureg, pc is third word */
			if (get4(map, (sp+2*sizeof(ulong)), (long *) &pc) < 0) {
				break;
			}
			sp +=  sizeof(struct Ureg);
		}
