		...
		if(m == 0) {
			if(p->as == ATEXT) {
				...
				if(p->from3.type == D_CONST) {
					if(p->from3.u0.aoffset & 3)
						diag("illegal origin\n%P", p);
					if(c > p->from3.u0.aoffset)
						diag("passed origin (#%lux)\n%P", c, p);
					else
						c = p->from3.u0.aoffset;
					p->pc = c;
				}
				if(p->from.sym != S)
					p->from.sym->value = c;
...
aclass(Adr *a)
{
	...
	case D_SPR:
		if(a->u0.aoffset == D_LR)
			return C_LR;
		if(a->u0.aoffset == D_XER)
			return C_XER;
		if(a->u0.aoffset == D_CTR)
			return C_CTR;
		return C_SPR;
	...
	case D_OREG:
		...
		case D_STATIC:
			...
			offset = a->sym->value + a->u0.aoffset - BIG;
