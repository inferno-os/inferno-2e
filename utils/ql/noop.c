...
noops(void)
{
	...
	for(p = firstp; p != P; p = p->link) {
		...
		case ABL:	/* ABCL? */
			if(curtext != P && curtext->from.sym != S && curtext->to.u0.aoffset >= 0) {
