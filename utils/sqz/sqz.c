...
static	uchar	prog[2*1024*1024];
static	uchar	outbuf[2*1024*1024];
...
main(int argc, char **argv)
{
	...
	if(txtlen > sizeof(prog) || datlen > sizeof(prog) || txtlen+datlen > sizeof(prog)){
		fprint(2, "sqz: executable too big\n");
		exits("size");
	}
	if(dflag){
		seek(fd, txtlen, 1);
...
squeeze(ulong *prog, int nw, uchar *out, ulong top)
{
	...
	fprint(2, "enc: %lud 4-bits, %lud 12-bits %lud 20-bits %lud 36-bits -- %d bytes\n",
		e1, e2, e3, e4, wp-out);
	return wp-out;
}
