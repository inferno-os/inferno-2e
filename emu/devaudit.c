...
find_nextb(long ii)
{
	...
	if (!bhdrok(b)) {
		...
		if (mptr != nil) {
			...
			dumpvl("area", (ulong *)((ulong)b & ~31)-4, 20);
		}
		else
...
_auditmemloc(char *str, void *v)
{
	...
	dumpvl("area", (ulong *)((ulong)v & ~31)-4, 20);
}
...
auditinit(void)
{
	char *callers = getenv("AUDIT_CALLERS");

	if (callers != nil)
		audit_callers = atoi(callers);
	mallocdbginit(audit_callers);
}

static Chan*
auditattach(void *spec)
{
...
auditread(Chan *c, void *buf, long n, ulong offset)
{
	...
	case Qmalloc:
		if (c->u.aux == nil) {
			c->u.aux = malloc(mallocpsz);
			if (c->u.aux == nil)
				error(Enomem);
			qlock(&memlock);
			mallocsummary(c->u.aux, mallocpsz);
			qunlock(&memlock);
		}
		return readstr(offset, buf, n, c->u.aux);
	case Qctl:
		if (caller_ix == nil)
			error(Enotenabled);
		qlock(&memlock);
