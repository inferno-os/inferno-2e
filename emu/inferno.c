...
Sys_read(void *fp)
{
	...
	/* Kludge for tcp/ip performance to avoid VM release/acquire:
	 * Do a nonblocked read. For IP device, kreadnb() may return some
	 * data, while for all other devices, it just returns 0
	 */
	cnt = kreadnb(fdchk(f->fd), f->buf->data, n);
	if (cnt == 0) {
		/* need to do a blocking read */
  		release();
		cnt = kread(fdchk(f->fd), f->buf->data, n);
  		acquire();
	}
	*f->ret = cnt;
}
...
Sys_write(void *fp)
{
	...
	/* Kludge for tcp/ip performance to avoid VM release/acquire:
	 * Do a nonblocked write. For IP device, kwritenb() may send some 
	 * data, while for all other devices, it just returns 0
	 */
	cnt = kwritenb(fdchk(f->fd), f->buf->data, n);
	if ((cnt != n) && (cnt >= 0)) {
		/* need blocked write */
  		release();
		cnt += kwrite(fdchk(f->fd), f->buf->data+cnt, n-cnt);
  		acquire();
	}
	*f->ret = cnt;
}
