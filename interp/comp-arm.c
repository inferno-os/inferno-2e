 ...
 *	as defined in utils/libmach/5.c. This is because when the acid
 ...
 *	the same values as the offsets in utils/libmach/5.c.
 */
...
enum
{
	...
	R12enum	= 12,		/* C's SB */
	R13enum	= 13,		/* C's SP */
	R14enum	= 14,		/* Link Register */
	R15enum	= 15,		/* PC */

	RLINK	= 14,

	RFP	= R9enum,	/* Frame Pointer */
	RMP	= R8enum,	/* Module Pointer */
	RTA	= R7enum,	/* Intermediate address for double indirect */
	RCON	= R6enum,	/* Constant builder */
	RREG	= R5enum,	/* Pointer to REG */
	RA3	= R4enum,	/* gpr 3 */
	RA2	= R3enum,	/* gpr 2 2+3 = L */
	RA1	= R2enum,	/* gpr 1 */
	RA0	= R1enum,	/* gpr 0 0+1 = L */
...
#define RETURN				DPI(AL, Add, RLINK, R15enum, 0, 0)				
#define CRETURN(C)				DPI(C, Add, RLINK, R15enum, 0, 0)				
...
urk(char *s)
{
	USED(s); error(exCompile);	//production
...
flushcon(int genbr)
{
	...
	for(i = 0; i < rcon.ptr; i++) {
		if(pass){
			...
			if(0)print("data %8.8lux %8.8lux (%8.8lux, ins=%8.8lux cpc=%8.8lux)\n", code, c->o, c->code, *c->code, c->pc);
			*c->code |= (disp&0xfff);
		}
		*code++ = c->o;
		c++;
...
//if(pass){print("%D\n", i); das(s, code-s);}
	...
	case IADDF:
		r = Adf;
	arithf:
		punt(i, SRCOP|DSTOP|THREOP, optab[i->op]); USED(r); break;
		opflld(i, Ldf, FA2);
		midfl(i, Ldf, FA4);
 ...
 */
	...
	LDW(AL, RCON, RTA, 8);
	CMP(AL, RA1, 0, 0, RTA);
	LDW(LT, RCON, R15enum, 12);	// v >= l[1] && v < l[2] => found; goto l[3]
	...
	LDW(AL, RLINK, R15enum, 4);		// goto (initial t)[n*3+1]
}
...
macret(void)
{
	...
	PATCH(cp4);
	MOV(R15enum, R14enum);		// call destroy(t(fp))
	MOV(RA0, R15enum);
	...
	DP(AL, Mov, 0, R15enum, 0, RA1);		// goto lr(Rfp), if compiled
...
macmcal(void)
{
	...
	CMPI(AL, RA1, 0, 0, 0);
	DP(NE, Mov, 0, R15enum, 0, RA0);	// return to compiled code
	mem(Stw, O(REG,FP),RREG, RFP);	// R.FP = RFP
...
macfram(void)
{
	...
	mem(Ldw, O(Type, initialize), RA3, R15enum);	// become t->init(RA2), returning RA2
