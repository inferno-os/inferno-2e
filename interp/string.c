...
OP(insc)
{
	...
	if(ss == H)
		ss = newstring(0);
	else if(D2H(ss)->ref > 1 || (expand && ss->len > 0))
		ss = splitc(R.d, expand);

	l = ss->len;
	if(l < 0 || expand) {
		l = -l;		/* 'expand' implies 'l' is <= 0 */
		if(v < l)
			ss->Srune[v] = r;
		else
		if(v == l && v < ss->max) {
			if(l == 0) /* 0-len ascii => rune */
				ss->max /= sizeof(Rune);
			ss->len = -(v+1);
			ss->Srune[v] = r;
		}
...
addstring(String *s1, String *s2, int append)
{
	...
	if(s1->len < 0) {
		...
		if (append && l <= s1->max)
			...
			ns = newstring((append ? (l+l/4) : l)*sizeof(Rune));	
			ns->max /= sizeof(Rune);
...
c2string(char *cs, int len)
{
	...
	if(isrune == 0) {
		nc = ecs - cs;
		s = newstring(nc);
		AUDITALLOC(1, getcallerpc(&cs), D2H(s));
