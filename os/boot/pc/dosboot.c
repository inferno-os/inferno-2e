...
static char *premature = "premature EOF\n";
...
dmddo:
	...
	p->dos = 0;
	b = (Dosboot *)p->iobuf;
	if(ishard && b->mediadesc != 0xF8 && b->mediadesc != 0xF0){
...
dosboot(Medium *dos, char *path)
{
	...
	if(GLLONG(ep->magic) != I_MAGIC){
		print("bad magic 0x%lux not an Inferno executable!\n", GLLONG(ep->magic));
		return -1;
	}
