...
static void
pinit(PPP *ppp, Pstate *p, int configrcvd)
{
	p->timeout = 0;
...
init(PPP* ppp)
{
	if(ppp->inbuf == nil){
		...
		kproc("ppptimer", ppptimer, ppp);
	}

	pinit(ppp, ppp->lcp, 0);
	newstate(ppp, ppp->lcp, Sreqsent);
}
	...
	case Lconfreq:
		...
		case Sackrcvd:
			...
			if(p->proto == Plcp && ppp->ipcp->state == Sidle){
				pinit(ppp, ppp->ipcp, 0);
				newstate(ppp, ppp->ipcp, Sreqsent);
			}
			break;
		case Sidle:
		case Sopened:
