...
fchar(void)
{
	...
	if(uidgc.find >= uidgc.flen) {
		...
		uidgc.flen = con_read(FID2, uidgc.uidbuf->iobuf,
			fscons.offset, BUFSIZE);
		if(uidgc.flen <= 0)
			return 0;
		fscons.offset += uidgc.flen;
	}
...
cmd_user(void)
{
	...
	memset(uid, 0, fsconf.nuid * sizeof(*uid));
	memset(uidspace, 0, fsconf.uidspace * sizeof(*uidspace));
	memset(gidspace, 0, fsconf.gidspace * sizeof(*gidspace));
...
ul1:
	...
	o -= strlen(name)+1;
	if(o < 0) {
		cprint("fsconf.uidspace(%ld) too small\n", fsconf.uidspace);
		goto initu;
	}
	...
	u++;
	if(u >= fsconf.nuid) {
		cprint("fsconf.nuid(%ld) too small\n", fsconf.nuid);
		goto initu;
	}
...
gl2:
	...
	if(n)
		gidspace[g++] = n;
	if(g >= fsconf.gidspace-2) {
		cprint("fsconf.gidspace(%ld) too small\n", fsconf.gidspace);
		goto initu;
	}
	if(c == '\n')
		goto gl3;
