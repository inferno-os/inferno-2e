...
extern ulong _ins(ulong);		/* return ulong to prevent unecessary compiler shifting */
...
/* Note:
 * The previous SA1100 version of io.h broke the tradition
 * of putting hardware registers into structures.  Instead,
 * macros were defined to directly access each hardware register.
 * This resulted in grossly inefficient code, and a need to 
 * always specify the "port" number for each register access
 * to any piece of hardware that had multiple devices.
 * (this was very noticeable in the original devuart.c,
 * and resulted in a lot of unnecessary overhead)
 * This has now been remedied, and the necessary remnants
 * of the old io.h has been included below so that upgrading
 * code to the new structures can be done incrementally.
 *
 * Bit by bit, code that depends on definitions below
 * should be converted, and those definitions should be removed
 * so that no new code is written using them.
 * When all code in os/sa1100 has been converted to use the
 * new io.h, the following definitions and this large comment
 * can be removed.  Until then, it should remain here
 * to let developers know what's going on 
 * (it may be necessary to add some additional definitions to the
 * first part of this file before the switchover is complete)
 */
 ...
 * GPIO Registers
 */
#define	GPIOBASE 	(SYSCTLbase+0x40000)

#define GPLR		(ulong *)(GPIOBASE+0x0)			/* Pin Level */
#define GPDR		(ulong *)(GPIOBASE+0x4)			/* Pin Direction */
#define GPSR		(ulong *)(GPIOBASE+0x8)			/* Output Set */
#define GPCR		(ulong *)(GPIOBASE+0xC)			/* Output Clear */
#define GRER		(ulong *)(GPIOBASE+0x10)		/* Rising Edge  */ 
#define GFER		(ulong *)(GPIOBASE+0x14)		/* Falling Edge  */
#define GEDR		(ulong *)(GPIOBASE+0x18)		/* Edge Status */
#define GAFR		(ulong *)(GPIOBASE+0x1C)		/* Alternate Func*/
...
/*
 * Peripheral Port Locations
 */

#define SERIALPort(n)	(PERPHbase+(0x10000*n))	/* Serial Ports */
#define PPCPort		(PERPHbase+0x50000)	/* Perph. Pin Ctl */
...
#define HSCR0		(ulong *)(SERIALPort(4)+0x60)	/* Ctl Reg 0 */
#define HSCR1		(ulong *)(SERIALPort(4)+0x64)	/* Ctl Reg 1 */
#define HSDR		(ulong *)(SERIALPort(4)+0x6c)	/* Data Reg */
#define HSSR0		(ulong *)(SERIALPort(4)+0x74)	/* Status Reg 0 */
#define HSSR1		(ulong *)(SERIALPort(4)+0x78)	/* Status Reg 1 */
...
#define SSCR0		(ulong *)(SERIALPort(7)+0x60)	/* Ctl Reg 0 */
#define SSCR1		(ulong *)(SERIALPort(7)+0x64)	/* Ctl Reg 1 */
#define SSDR		(ulong *)(SERIALPort(7)+0x6C)	/* Data Reg */
#define SSSR		(ulong *)(SERIALPort(7)+0x74)	/* Status Reg */
